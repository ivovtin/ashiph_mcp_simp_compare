void n105_npe_sigma()
{
   ...
   TProfile* pr2 = new TProfile("#sigma","#sigma",100,0,6.5,0,5);

   const int ncounts = 20;
   float npethr_step = 0.2;
   
   TProfile* Ksigma[ncounts];
   char name1[ncounts];
   for (int j=0; j<ncounts; j++)
   {
      sprintf(name1,"Ksigma%d",j);
      Ksigma[j] = new TProfile(name1,"",250,-0.5,990.5);
   }
   ...
   for (int i=0; i<1000; i++)
   {
      ...
      for( int k=0; k<ncounts; k++ )
      {
         ...
         if( i>200 && i<300 ) pr2->Fill((float)Npethr[k],abs(sqrt(2.)*(TMath::ErfInverse(1-2*underthr_eff_pi[k])+TMath::ErfInverse(1-2*(1-eff[k])))));
      }
      ...
      //Ksigma->Fill((float)i,abs(sqrt(2.)*(TMath::ErfInverse(1-2*underthr_eff_pi[5])+TMath::ErfInverse(1-2*(1-eff[5])))));
   }
   ...
   for( int i=0; i<ncounts; i++ )
   {
      ...
      c2->SaveAs(TString::Format("out/Ksigma_%d.png",i).Data());
   }
